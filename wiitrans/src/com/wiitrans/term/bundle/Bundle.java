package com.wiitrans.term.bundle;
...
public class Bundle extends Thread implements IBundle, IServer {
	...
	public String GetBundleId() {
		return BundleConf.TERM_BUNDLE_ID;
	}
	...
	public int Request(JSONObject msg) {
		...
		Log4j.log("term bundle " + msg.toString());
	...
	public int Response(Client client) {
		...
		case Const.BUNDLE_REGISTER: {
			...
			// Registe bundle.
			if (0 == (bid.compareTo(BundleConf.TERM_BUNDLE_ID))) {
				if (_client != null) {
			...
			if (result != null) {
				...
				default: {
					...
					if (_res != null) {
						...
						Log4j.error("term service bundle callback is null");
					}
					break;
				}
				}
		...
		default:
			Log4j.error("term service bundle state[" + state + "] error");
			break;
		}
	...
	public void run() {
		...
		svr.SetPort(BundleConf.TERM_BUNDLE_PORT);
