...
public class TMDEDE extends TMENUS {
	private LANGUAGE_NAME _name = LANGUAGE_NAME.GERMAN;
	private LANGUAGE_TYPE _type = LANGUAGE_TYPE.LETTER;
	private LANGUAGE_COUNTRY _langcountry = LANGUAGE_COUNTRY.DEDE;
	private String _slangcountry = LangConst.LANGUAGE_COUNTRY_DEDE;
		...
		if (word.length() <= 10) {
			...
			for (int i = 0; i < cs.length; i++) {
				if (cs[i] >= 'a' && cs[i] <= 'z') {
					...
					letters[i] = cs[i] - '0';
				} else if (cs[i] == 'ä' || cs[i] == 'Ä') {
					letters[i] = 36;
				} else if (cs[i] == 'ö' || cs[i] == 'Ö') {
					letters[i] = 37;
