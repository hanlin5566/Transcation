...
public class TMESES extends TMENUS {
	private LANGUAGE_NAME _name = LANGUAGE_NAME.SPANISH;
	private LANGUAGE_TYPE _type = LANGUAGE_TYPE.LETTER;
	private LANGUAGE_COUNTRY _langcountry = LANGUAGE_COUNTRY.ESES;
	private String _slangcountry = LangConst.LANGUAGE_COUNTRY_ESES;
	...
	public int IsLetter(char c) {
		// 1小写2大写3数字0非字母
		int result;
		if (c >= 'a' && c <= 'z' || c == 'á' || c == 'é' || c == 'í'
				|| c == 'ó' || c == 'ú' || c == 'ü' || c == 'ñ') {
			result = 1;
		} else if (c >= 'A' && c <= 'Z' || c == 'Á' || c == 'É' || c == 'Í'
				|| c == 'Ó' || c == 'Ú' || c == 'Ü' || c == 'Ñ') {
			result = 2;
		} else if (c >= '0' && c <= '9') {
			result = 3;
		...
		if (word.length() <= 10) {
			...
			for (int i = 0; i < cs.length; i++) {
				if (cs[i] >= 'a' && cs[i] <= 'z') {
					...
					letters[i] = cs[i] - '0';
				} else if (cs[i] == 'á' || cs[i] == 'Á') {
					letters[i] = 36;
				} else if (cs[i] == 'é' || cs[i] == 'É') {
					letters[i] = 37;
				} else if (cs[i] == 'í' || cs[i] == 'Í') {
					letters[i] = 38;
				} else if (cs[i] == 'ó' || cs[i] == 'Ó') {
					letters[i] = 39;
				} else if (cs[i] == 'ú' || cs[i] == 'Ú') {
					letters[i] = 40;
				} else if (cs[i] == 'ü' || cs[i] == 'Ü') {
					letters[i] = 41;
				} else if (cs[i] == 'ñ' || cs[i] == 'Ñ') {
					letters[i] = 42;
				} else if (cs[i] == '-') {
					letters[i] = 61;
				} else if (cs[i] == '\'') {
