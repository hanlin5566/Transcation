package com.wiitrans.msg.wordfilter;
import java.io.File;
...
public class WordFilterUtil {
	...
	static {
		...
		//读取配置文件指定的文件
		try {
			is = new FileInputStream(new File(BundleConf.FILTER_WORD_CONF_URL));
		} catch (Exception e) {
			// TODO: handle exception
		    	Log4j.error(e);
			is = WordFilterUtil.class.getResourceAsStream("/words.dict");
		}
		try {
	...
	private static PunctuationOrHtmlFilteredResult filterPunctation(String originalString){
		...
		for(int i=0;i<originalString.length();i++){
			String c = String.valueOf(originalString.charAt(i)).toLowerCase();
