package com.wiitrans.frag.spout;
...
//		app.Parse();
		...
		_taskCollector = new TaskCollector(BundleConf.BUNDLE_REPORT_IP,
				...
				BundleConf.FRAGMENTATION_BUNDLE_ID);
	...
	public void nextTuple() {
		...
		if (tasks != null) {
			for (JSONObject task : tasks) {
				String fid = Util.GetStringFromJSon("fid", task);
				String nid = Util.GetStringFromJSon("nid", task);
				Log4j.debug("spout " + task.toString());
				if (fid != null && nid != null) {
					_collector.emit(new Values(fid, task.toString()));
				} else {
					String aid = Util.GetStringFromJSon("aid", task);
					if (aid != null
							&& (aid.equals("close") || aid.equals("tmx"))) {
						_collector.emit(new Values(0, task.toString()));
					} else {
						SendToPHP(task, "FAILED");
						Log4j.warn("frag spout aid is null. ");
					}
				}
			}
		} else {
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		...
		declarer.declare(new Fields("fid", "content"));
	}
}